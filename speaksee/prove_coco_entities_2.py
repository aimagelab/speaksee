from speaksee.data import ImageField, CocoMultipleAssociatedDetectionsField, TextField
...
det_field = CocoMultipleAssociatedDetectionsField(detections_path='/tmp/coco_det_feats_loc.hdf5',
...
from speaksee.models import CocoEntitiesAttention
model = CocoEntitiesAttention(len(text_field.vocab), h2_fist_lstm=opt.h2_fist_lstm,
    ...
    for e in range(start_epoch, 50):
        ...
        with tqdm(desc='Epoch %d - train' % e, unit='it', total=len(iter(dataloader_train))) as pbar:
            for it, (images, detections, det_ids, captions) in enumerate(iter(dataloader_train)):
                ...
                loss_att = loss_fn_att(att_out[:, :, 0].view(-1, 1), (det_ids[:, 1:, 0] == 0).float().view(-1, 1))
